...
namespace SimpleGameLib.Events
{
    ...
    public class EventMap
    {
        ...
        public EventObject getEventByName(String name)
        {
            ...
            {
                Log.getInstance().log("@Folder:Events, Class:EventMap, Log Type: Error, Line No: 50, " + "EventMap " + this.ToString() + " could not find the event " + name + " st " + System.Environment.StackTrace);
                return (null);
            }
        }
        ...
        public bool killEventByName(String name)
        {
            ...
            {
                Log.getInstance().log("@Folder:Events, Class:EventMap, Log Type: Error, Line No: 69, " + "EventMap " + this.ToString() + " the event " + name + " could not kill the event st " + System.Environment.StackTrace);
                return (false);
            }
        }
        ...
        public void checkCollisionsWithPlayer(PythonObject pyth, Rectangle player)
        {
            ...
            while (iter.MoveNext())
            {
                if ((iter.Current.Triggered == Trigger.player_touch) && (iter.Current.getCollisionBox().Intersects(player)))
                {
                    ...
                    //Execute the event
                    pyth.executeF(ScriptKeeper.getInstance().getScript(iter.Current.ScriptFile));
                }
            }
        ...
        public void removeDead()
        {
            ...
            {
                events.Remove(ev);
                Log.getInstance().log("@Folder:Events, Class:EventMap, Log Type: Program Run Log, Line No: 119, " + "EventMap removing the event " + ev);
            }
        ...
        {
            Log.getInstance().log("@Folder:Events, Class:EventMap, Log Type: Program Run Log, Line No: 130, " + "EventMap " + this.ToString() + " from " + source + " starting to load the events ");
            ...
            Log.getInstance().log("@Folder:Events, Class:EventMap, Log Type: Program Run Log, Line No: 137 " + "EventMap " + this.ToString() + " finished loading events " + source);
