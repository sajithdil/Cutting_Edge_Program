...
namespace SimpleGameLib.SWRenderer
{
    ...
    public class Style
    {
        ...
        {
            Log.getInstance().log("@Folder:SWRenderer, Class:Style, Log Type: Program Run Log, Line No: 41, " + "Style starting to load the file style " + file);
            XMLReader reader = new XMLReader(file);
            reader.process("style");
            reader.getObjectSpace().convert(this.convert);
            Log.getInstance().log("@Folder:SWRenderer, Class:Style, Log Type: Program Run Log, Line No: 45, " + "Style finished loading file style " + file);
