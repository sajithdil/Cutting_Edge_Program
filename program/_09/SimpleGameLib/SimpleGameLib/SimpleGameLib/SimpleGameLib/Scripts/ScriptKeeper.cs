...
namespace SimpleGameLib
{
    ...
    public class ScriptKeeper
    {
        ...
        public String getScript(String alias)
        {
            ...
            {
                Log.getInstance().log("@Folder:Scripts, Class:ScriptKeeper, Log Type: Error, Line No: 42, " + "ScriptKeeper : Could find the script " + alias + "st " + System.Environment.StackTrace);
        ...
        public void init(String source)
        {
            ...
            Log.getInstance().log("@Folder:Scripts, Class:ScriptKeeper, Log Type: Program Run Log, Line No: 82, " + "Scriptkeeper : Starting to load scripts ");
            XMLReader reader = new XMLReader(source);
            reader.process("script");
            reader.getObjectSpace().convert(this.convert);
            Log.getInstance().log("@Folder:Scripts, Class:ScriptKeeper, Log Type: program Run Log, Line No: 86, " + "Scriptkeeper : Finished loading scripts ");
        }
