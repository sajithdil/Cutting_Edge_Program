...
namespace SimpleGameLib
{
    ...
    public class WorldsKeeper
    {
        ...
        public World getWorld(String id)
        {
            ...
            {
                Log.getInstance().log("@WorldsKeeper could not load the world " + id+" st "+System.Environment.StackTrace);
        ...
        {
            Log.getInstance().log("@WorldsKeeper loading the worlds");
            XMLReader reader = new XMLReader(file);
            reader.process("worlds");
            reader.getObjectSpace().convert(this.convert);
            Log.getInstance().log("@WorldsKeeper finished loading the worlds");
        }
        ...
        public World getCurrent()
        {
            if (current == null)
            {
                Log.getInstance().log("@WorldsKeeper Attempt to request the current world ,when the current world does not exist"+System.Environment.StackTrace);
                System.Environment.Exit(-1);
                return null;
            }
